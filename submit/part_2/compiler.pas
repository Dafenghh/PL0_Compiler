program  PL0;
{支持read, write函数的PL0编译程序}
const
    kReservedWords = 13; {保留字的个数}
...
type
    ...
    FunctionCode = (LIT, OPR, LOD, STO, CAL, INT, JMP, JPC, RED, WRT); {functions}
...
begin {Statement}
    ...
    else if curr_symbol = READSYM then {处理读入语句}
    begin
        GetSymbol;
        if curr_symbol <> LPAREN then error(10)
        else
        begin
            repeat
                GetSymbol;
                if curr_symbol <> IDENT then error(4)
                else
                begin
                    i := position(id);  {在符号表中查id, 返回id在符号表中的入口}
                    if i = 0 then error(11) {若在符号表中查不到id, 则出错}
                    else if table[i].kind <> kVariable then error(12) {对非变量赋值, 则出错}
                    else
                        with table[i] do GenerateCode(RED, lev-level, adr);
                end;
                GetSymbol;
            until curr_symbol <> COMMA;
            if curr_symbol <> RPAREN then error(22);
            GetSymbol;
        end
    end
    else if curr_symbol = WRITESYM then {处理输出语句}
    begin
        GetSymbol;
        if curr_symbol <> LPAREN then error(10)
        else
        begin
            repeat
                GetSymbol;
                Expression([RPAREN,COMMA] + symbol_set);
                GenerateCode(WRT,0,0);
            until curr_symbol <> COMMA;
            if curr_symbol <> RPAREN then error(22);
            GetSymbol;
        end   
    ...
    repeat
        ...
        with i do
            case func of
                ...
                RED :
                    begin
                        read(pl0_input, stack[BaseOf(level)+adr]);
                    end;
                WRT :
                    begin
                        writeln(stack[top]); 
...
begin  {主程序}
    ...
    words[12] := 'while'; 
    words[13] := 'write';
    word_symbol[1] := BEGINSYM;   
    word_symbol[2] := CALLSYM; 
    word_symbol[3] := CONSTSYM;   
    word_symbol[4] := DOSYM;
    word_symbol[5] := ENDSYM;    
    word_symbol[6] := IFSYM;
    word_symbol[7] := ODDSYM;    
    word_symbol[8] := PROCSYM;
    word_symbol[9] := READSYM;
    word_symbol[10] := THENSYM;    
    word_symbol[11] := VARSYM;
    word_symbol[12] := WHILESYM;
    word_symbol[13] := WRITESYM;
    ...
    {说明语句的开始符号}
    stat_begin_symbols := [BEGINSYM, CALLSYM, IFSYM, WHILESYM, READSYM, WRITESYM];
    {语句的开始符号}
